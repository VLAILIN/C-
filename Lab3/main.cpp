...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	{
		MyString s1("AAA"), s2;
		s2=s1; //ФФВ s2.operator=(s1);
		s1="CCC";
		s2= MyString("tmp"); //move оператор
		s1=s1;
		stop
	}
	...
	stop
		...
		//Например:
			...
			bd["Ivanov"] = MyData(MALE, 30, "worker", 40000);	//если Ivanov-а в базе еще нет, то
			//добавление, если уже есть, то замена его реквизитов

			bd["Petrov"] = MyData(MALE, 22, "worker", 20000);
			bd["Sidorov"] = MyData(MALE, 39, "worker", 50000);
			cout << bd;	//вывод информации обо всех сотрудниках

			bd["Ivanov"] = MyData(MALE, 99, "director", 99000);
			cout<<bd;	//вывод информации обо всех сотрудниках
			...
			//А также:
			 BD bdnew = bd;
			 bdnew["Smirnova"] = MyData(FEMALE, 22, "worker", 25000);
			 cout << bdnew;
			 stop;
			//
			//bdnew = bd;
	...
	{
		MyData m1;
		MyData m2(MALE, 40, "worker", 40000);
		m1 = m2;
		cout << m1; // << endl;
		cout << m2; // << endl;
		stop
